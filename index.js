...
app.get("/edit/:id", async (req, res) => {
  ...
  res.render("admin/updateForm.ejs", {
    songIdToUpdate,
    songTitle,
    songAlbum,
    songArtist,
...
app.post("/admin/update-song/:id", upload.single("song"), async (req, res) => {
  ...
  try {
    ...
    await db.query(updateSongQuery, [
      songTitle,
      songArtist,
      songAlbum,
      songcategory,
...
app.get("/admin/album/:albumId/add-song", async (req, res) => {
  ...
  const album = await db.query("SELECT * FROM albums WHERE album_id = $1", [
    albumId,
  ]);
...
app.post("/admin/album/:albumId/add-songs", async (req, res) => {
  ...
  try {
    ...
    for (const songId of songIds) {
      await db.query(
        "INSERT INTO album_songs (album_id, song_id) VALUES ($1, $2) ON CONFLICT (album_id, song_id) DO NOTHING",
        [albumId, songId]
      );
    }
...
app.post(
  ...
  async (req, res) => {
    ...
    await db.query(
      "INSERT INTO promotions (song_title, artist_name, category, release_date, file_path, description, status) VALUES ($1, $2, $3, $4, $5, $6, $7)",
      [
        songTitle,
        artistName,
        category,
...
app.get("/admin/view-promotions", async (req, res) => {
  try {
    ...
    const result = await db.query(
      "SELECT * FROM promotions ORDER BY release_date DESC"
...
app.get("/admin/pending-promotions", async (req, res) => {
  try {
    ...
    res.render("admin/pendingPromotions.ejs", {
...
app.get("/admin/albums/viewAlbum/:id", async (req, res) => {
  ...
  try {
    ...
    const songsResult = await db.query(
      "SELECT * FROM songs WHERE id IN (SELECT song_id FROM album_songs WHERE album_id = $1)",
      [albumID]
    );
...
`);
    ...
    req.session.error =
      "An error occurred while removing the song from the album.";
    res.redirect(`/admin/album/${albumID}/edit`);
  }
});

// /admin/view-reports
app.get("/admin/view-reports", (req, res) => {
  let totalSongsAdded = 0;
  let totalSongDeleted = 0;
  let totalAlbumCreated = 0;
  let totalAlbumDeleted = 0;
  let totalApprovedPromotions = 0;
  let totalPendingPromotions = 0;
  let totalRejectedPromotions = 0;

  res.render("admin/viewReports.ejs", {
    totalSongsAdded,
    totalSongDeleted,
    totalAlbumCreated,
    totalAlbumDeleted,
...
app.post("/submit-contact", async (req, res) => {
  try {
    ...
    let sql =
      " INSERT INTO contact_us (name, email, subject, message) VALUES ($1, $2, $3, $4)";
